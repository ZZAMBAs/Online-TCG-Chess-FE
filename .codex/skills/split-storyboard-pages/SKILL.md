...
description: Online-TCG-Chess-FE에서 BE 요구사항과 PRD 확인 뒤 storyboard 구조 단계의 페이지 분리안을 만든다. docs/design/storyboard-pages.md와 version 2 storyboard-manifest의 pages, 구조/시각 review 초기 상태, 요구사항 추적과 시각 참고 후보를 생성·갱신할 때 사용한다.
---
...
1. 먼저 `$spec-read`와 `$prd-read`로 BE 요구사항과 PRD 최신성을 확인한다. 하나라도 실패하면 작업을 중단한다.
...
7. `storyboard-pages.md`와 manifest를 함께 갱신하고 `workflow_stage`를 `structure-draft` 또는 `structure-review`로 둔다.
...
- 신규 page에는 `review.structure: draft`, `review.visual: blocked`, `fidelity: structure`, `visual_reference: null`, 빈 `representative_states`와 `component_patterns`를 기본값으로 기록한다.
