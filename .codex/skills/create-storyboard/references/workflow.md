...
2. `storyboard-manifest.json`과 `storyboard-pages.md`를 읽어 기존 상태를 복원한다.
3. 페이지 분리안이 없거나 요구사항 변경으로 오래되었으면 `$split-storyboard-pages`로 갱신한다.
4. 만들거나 수정할 page id를 하나 고른다.
5. 새 페이지라면 manifest와 fragment 파일명/본문을 먼저 검색해 중복 여부를 기록한다.
6. `$create-storyboard-page <page-id>`로 해당 fragment만 작성하거나 수정한다.
7. 사용자가 승인하면 해당 page의 `status`를 `approved`로 바꾼다. 반려면 `needs-revision`과 반려 이유를 남긴다.
8. 통합이 필요하면 `scripts/build_storyboard.py`를 실행한다.
9. `rg "<iframe|srcdoc" docs/design/storyboard.html docs/design/storyboard/fragments`로 금지 요소를 확인한다.
10. manifest의 모든 page id가 `docs/design/storyboard.html`에 포함되는지 확인한다.
11. 브라우저 캡처로 확인했다면 검증 결과만 요약하고 캡처 파일은 삭제한다.
