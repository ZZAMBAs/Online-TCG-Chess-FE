---
name: tdd-red
description: Online-TCG-Chess-FE의 로컬 feature 또는 foundation 이슈의 Given-When-Then AC를 FE 테스트로 옮겨 TDD RED 단계를 수행해야 할 때 사용한다. unit/component/API-client/STOMP-client/계약 또는 공통 UI primitive 관점 테스트와 RED 결과만 생성·수정하며 production 구현, 앱 skeleton, package 설정, E2E는 수정하지 않는다.
---
...
`slice_type: foundation`은 feature PRD 대신 issue가 참조하는 approved root TRD, fixed architecture, 관련 디자인 기준을 읽는다. 필요한 기반 source가 없으면 테스트를 만들지 않고 `blocked`로 기록한다.
...
- foundation UI는 token의 구체 CSS 값보다 primitive의 public variant, semantic DOM, keyboard/focus, disabled/loading/error 상태와 후속 feature가 소비할 surface를 검증한다.
