---
name: prd-read
description: Online-TCG-Chess-FE에서 BE가 생성한 전체 PRD와 기능별 PRD 원천을 확인, 분석, 구현 계획, 이슈 분리, TRD/스토리보드/아키텍처 판단에 사용한다. BE 저장소 master의 docs/prd.md, docs/features/{feature}/prd.md, docs/traceability.md를 원격 최신성 확인 후 .cache/prd-read/docs에 저장하며, 원격 확인 실패 시 캐시가 있어도 PRD 기반 작업을 중단한다.
---
...
- FE 저장소에는 PRD projection을 만들지 않는다. PRD의 로컬 참조 경로는 `.cache/prd-read/docs/`이며, 권위 원천은 항상 BE repo다.
...
3. helper가 성공하면 출력된 manifest와 `.cache/prd-read/docs/`의 캐시 파일을 읽어 PRD를 분석한다.
...
- 캐시가 최신이면 `.cache/prd-read/docs/`와 `.cache/prd-read/manifest.json`을 재사용한다.
- 원격 HEAD가 바뀌었거나 캐시가 없으면 BE PRD 산출물을 다시 받는다.
- 새로 받은 PRD 산출물은 `.cache/prd-read/docs/`에 저장한다. FE repo `docs/`에는 PRD를 복사하지 않는다.
...
- feature 인자가 있으면 `.cache/prd-read/docs/features/{feature}/prd.md`가 정확히 있어야 한다. 유사 feature를 추측하지 않는다.
...
- `.cache/prd-read/docs/`의 PRD 캐시는 후속 `$create-trd`, `$create-issues-adr` 입력으로 사용한다. 캐시 내용은 `$prd-read`가 관리하며 손으로 수정하지 않는다.
...
- `manifest.json`은 캐시된 PRD 경로와 BE commit/hash를 기록한다.
