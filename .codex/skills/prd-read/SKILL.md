---
name: prd-read
description: Online-TCG-Chess-FE에서 BE가 생성한 전체 PRD와 기능별 PRD 원천을 확인, 분석, 구현 계획, 이슈 분리, TRD/스토리보드/아키텍처 판단에 사용한다. BE 저장소 master의 docs/prd.md, docs/features/{feature}/prd.md, docs/traceability.md를 원격 최신성 확인 후 .cache/prd-read에만 캐시하고, 원격 확인 실패 시 캐시가 있어도 PRD 기반 작업을 중단한다.
---
...
- FE 저장소에 PRD 원천을 권위 문서로 복사하지 않는다.
...
3. helper가 성공하면 출력된 manifest와 캐시 파일을 읽어 PRD를 분석한다.
